...
public class ServicioCatalogoMock implements IServicioCatalogo {
    ...
    public Mueble actualizarMueble(Mueble mueble) {
        Mueble result = null;
        int index = getIndex(mueble.getReferencia());
        if (index != -1) {
            muebles.set(index, mueble);
            result = muebles.get(index);
        }
        return result;
    }

    public int getIndex(String referencia) {
        int result = -1;
        for (int i = 0; i < muebles.size(); i++) {
            if (muebles.get(i).getReferencia().equalsIgnoreCase(referencia)) {
                result = i;
            }
        }
        return result;
